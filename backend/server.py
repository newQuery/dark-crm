...
async def generate_invoice_pdf(invoice_id: str, current_user: User = Depends(get_current_user)):
    ...
    # Add logo (download from URL)
    try:
        logo_url = "https://customer-assets.emergentagent.com/job_nextcrm-hub/artifacts/0kh97peq_1.png"
        response = requests.get(logo_url)
        if response.status_code == 200:
            logo_buffer = BytesIO(response.content)
            logo = Image(logo_buffer, width=2*inch, height=0.5*inch)
    ...
    # Invoice info table
    invoice_info_data = [
        ['Invoice Date:', datetime.fromisoformat(invoice['issued_date']).strftime('%B %d, %Y')],
        ['Due Date:', datetime.fromisoformat(invoice['due_date']).strftime('%B %d, %Y')],
    ...
    if project:
        elements.append(Paragraph("PROJECT", heading_style))
        elements.append(Paragraph(f"<b>{project['title']}</b>", styles['Normal']))
        elements.append(Spacer(1, 20))
    ...
    items_data = [
        ['Description', 'Amount'],
        [invoice.get('project_title', project['title'] if project else 'Services Rendered'), f"${invoice['amount']:,.2f}"],
    ]
    
    items_table = Table(items_data, colWidths=[4*inch, 1.5*inch])
    items_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#00C676')),
    ...
    total_table.setStyle(TableStyle([
        ...
        ('FONTSIZE', (0, 0), (-1, -1), 14),
        ('TEXTCOLOR', (0, 0), (-1, -1), colors.HexColor('#00C676')),
    ...
    if invoice['status'] == 'paid' and invoice.get('paid_at'):
        paid_text = f"<b>PAID</b> on {datetime.fromisoformat(invoice['paid_at']).strftime('%B %d, %Y')}"
        ...
        elements.append(Paragraph(paid_text, paid_style))
